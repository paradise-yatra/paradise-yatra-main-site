...
export default function DestinationPage({ params }: DestinationPageProps) {
  ...
  useEffect(() => {
    const fetchDestination = async () => {
      try {
        ...
        if (!foundDestination) {
          console.error('Destination not found. Available destinations:', destinationsArray.map((d: any) => ({
            name: d.name,
            slug: d.slug,
            _id: d._id
          })));
                    ...
                    <CardContent className="p-4 sm:p-6">
                      ...
                      <Button 
                        onClick={() => {
                          ...
                          router.push(`/destinations/${pkgSlug}`);
                        }}
                        className="w-full bg-blue-600 text-white hover:bg-blue-700 py-3 text-sm font-bold rounded-lg shadow-md transition-all duration-300 transform hover:scale-105"
                      >
                        View Details
  ...
  return (
    ...
    >
      ...
      <PackageHeader 
        title={destination.name}
        subtitle={destination.description || destination.shortDescription || "Discover this amazing destination"}
        duration={destination.duration || ""}
        location={destination.location || ""}
        rating={destination.rating?.toString() || "0"}
        totalReviews={0}
