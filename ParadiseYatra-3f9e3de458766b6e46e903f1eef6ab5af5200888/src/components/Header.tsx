...
import { Menu, Shield, Users, Headphones, Star, Phone, ChevronDown, X, MapPin, Calendar, Search, Globe, Heart, Plane, Waves, Mountain as MountainIcon, Building, TreePine, Camera } from "lucide-react";
...
const Header = () => {
  ...
  useEffect(() => {
    (window as { stopHeaderLoading?: () => void }).stopHeaderLoading = stopNavigationLoading;
              ...
              {trustItems.map((item, index) => (
                ...
                >
                  <motion.span 
                    ...
                    transition={{ duration: 1.8, repeat: Infinity, repeatType: "mirror", delay: index * 0.2 }}
                    className="flex items-center space-x-1"
                  >
                      ...
                      >
                        ...
                        >
                          {item.submenu.map((subItem, subIndex) => {
                            ...
                            if (subItem.fixedDepartures && subItem.fixedDepartures.length > 0) {
                              return (
                                <div key={subIndex} className="border-b border-gray-100 last:border-b-0">
                                  <div className="px-4 py-3 bg-gradient-to-r from-blue-50 to-purple-50">
                                    <div className="flex items-center">
                                      ...
                                      <span className="flex-1 font-medium text-blue-700">{subItem.name}</span>
                                    </div>
                                  </div>
                                  ...
                                  <div className="max-h-48 overflow-y-auto scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-gray-100 relative">
                                    ...
                                    {subItem.fixedDepartures.slice(0, 4).map((departure, depIndex) => (
                                      <a
                                        key={depIndex}
                                      ...
                                      >
                                        ...
                                        <div className="flex-1 min-w-0">
                                          <div className="font-medium text-sm truncate">{departure.title}</div>
                                          <div className="text-xs text-gray-500 truncate">{departure.destination}</div>
                                          <div className="flex items-center space-x-2 mt-1">
                                            <span className="text-xs text-blue-600 font-medium">{departure.duration}</span>
                                            <span className="text-xs text-gray-400">•</span>
                                            <span className="text-xs text-green-600 font-medium">₹{departure.price.toLocaleString()}</span>
                                            {departure.discount > 0 && (
                                              <>
                                                <span className="text-xs text-gray-400">•</span>
                                                <span className="text-xs text-red-600 font-medium">{departure.discount}% OFF</span>
                                              </>
                                            )}
                                          </div>
                                        </div>
                            ...
                            if (item.name === "India Tour Package" && subItem.destinations && subItem.destinations.length > 0) {
                              return (
                                <div key={subIndex} className="border-b border-gray-100 last:border-b-0">
                                  <div className="px-4 py-3 bg-gradient-to-r from-blue-50 to-purple-50">
                                    <a 
                                      href={`/packages/india/${subItem.name.toLowerCase().replace(/\s+/g, '-')}`}
                                    ...
                                    >
                                      <DestinationIcon className="w-4 h-4 mr-3 text-blue-500" />
                                      <span className="flex-1 font-medium text-blue-700 hover:text-blue-800">{subItem.name}</span>
                            ...
                            if (item.name === "International Tour" && subItem.destinations && subItem.destinations.length > 0) {
                              return (
                                <div key={subIndex} className="border-b border-gray-100 last:border-b-0">
                                  <div className="px-4 py-3 bg-gradient-to-r from-blue-50 to-purple-50">
                                    <a 
                                      href={`/packages/international/${subItem.name.toLowerCase().replace(/\s+/g, '-')}`}
                                      onClick={() => setIsNavigating(true)}
                                      className="flex items-center hover:bg-gradient-to-r hover:from-blue-100 hover:to-purple-100 rounded-lg p-2 transition-all duration-200 relative group"
                                    >
                                      <DestinationIcon className="w-4 h-4 mr-3 text-blue-500" />
                            ...
                            return (
                              <div key={subIndex} className="border-b border-gray-100 last:border-b-0">
                                ...
                                {subItem.destinations && subItem.destinations.length > 0 && item.name !== "India Tour Package" && item.name !== "International Tour" && (
                                  <div className="bg-gray-50/50 px-4 py-2 max-h-32 overflow-y-auto scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-gray-100 relative">
                                    ...
                                    <div className="space-y-1">
                                      {subItem.destinations.slice(0, 5).map((dest, destIndex) => (
                                        <a
                                          key={destIndex}
                                          href={dest.type === 'package' ? `/itinerary/${dest.id}` : `/destinations/${dest.id}`}
                              ...
                              return (
                                <div key={subIndex} className="space-y-2">
                                  <div className="px-3 py-2 bg-gradient-to-r from-blue-50 to-purple-50 rounded-lg">
                                    <div className="flex items-center">
                                      <DestinationIcon className="w-4 h-4 mr-3 text-blue-500" />
                                      <span className="font-semibold text-blue-700">{subItem.name}</span>
                                    </div>
                                  </div>
                                    ...
                                    {subItem.fixedDepartures.slice(0, 4).map((departure, depIndex) => (
                                      ...
                                      >
                                        <div className="flex items-start space-x-3">
                                          ...
                                          <div className="flex-1 min-w-0">
                                            <div className="font-medium text-sm text-gray-800 truncate">{departure.title}</div>
                                            <div className="text-xs text-gray-500 truncate">{departure.destination}</div>
                                            <div className="flex items-center space-x-2 mt-1">
                                              <span className="text-xs text-blue-600 font-medium">{departure.duration}</span>
                                              <span className="text-xs text-gray-400">•</span>
                              ...
                              <div key={subIndex} className="space-y-2">
                                ...
                                {subItem.destinations && subItem.destinations.length > 0 && item.name !== "India Tour Package" && item.name !== "International Tour" && (
                                  <div className="ml-6 pl-3 border-l-2 border-blue-200 space-y-1 max-h-32 overflow-y-auto scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-gray-100 relative">
                                    ...
                                    <div className="text-xs text-gray-500 font-medium mb-1">Popular Destinations:</div>
                                    {subItem.destinations.slice(0, 4).map((dest, destIndex) => (
                                      <motion.a
                                        key={destIndex}
                                        href={dest.type === 'package' ? `/itinerary/${dest.id}` : `/destinations/${dest.id}`}
                                        whileHover={{ x: 3 }}
                                    ...
                                    {subItem.destinations.length > 4 && (
                                      <div className="text-xs text-blue-600 hover:text-blue-700 cursor-pointer pt-1">
                                        +{subItem.destinations.length - 4} more
                                      </div>
                                    )}
                                  </div>
                                )}
      ...
      {isNavigating && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[60] flex items-center justify-center">
          <div className="bg-white rounded-2xl p-8 shadow-2xl max-w-sm mx-4 text-center">
            ...
            <h3 className="text-lg font-semibold text-gray-800 mb-2">Loading Packages</h3>
            <p className="text-gray-600 text-sm mb-4">Please wait while we fetch the best packages for you...</p>
