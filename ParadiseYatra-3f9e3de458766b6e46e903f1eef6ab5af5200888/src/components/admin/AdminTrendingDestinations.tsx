...
// import React, { useState, useEffect } from "react";
// import { Crown, Star, MapPin, Clock, Edit, Trash2, Plus, Eye, Loader2, Upload, Image as ImageIcon } from "lucide-react";
// import Image from "next/image";
// import { toast } from "react-toastify";
// import ImageUpload from "@/components/ui/image-upload";
// import { getImageUrl } from "@/lib/utils";
...
//     state: "",
//     tourType: "",
//     category: "Trending Destinations", // Default to Trending Destinations
//     rating: "",
//     price: "",
//     duration: "",
//     highlights: "",
...
//   const handleCountryChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
//     const countryIso2 = e.target.value;
//     const selectedCountry = filteredCountries.find(c => c.iso2 === countryIso2);
    
//     setSelectedCountryIso2(countryIso2);
//     setFormData(prev => ({ 
//       ...prev, 
//       country: selectedCountry?.name || countryIso2, 
//       state: '' 
//     }));
...
//           images: [imageUrl], // Send URL directly
//           highlights: formData.highlights ? formData.highlights.split(',').map(h => h.trim()) : []
//         };
...
//         console.log('Created destination with URL:', newDestination);
        
//         await fetchTrendingDestinations();
//         resetForm();
//         toast.success('Trending destination added successfully with image URL!');
//         return;
//       }

//       // File upload method (existing code)
...
//       await fetchTrendingDestinations();
//       resetForm();
//       toast.success('Trending destination added successfully! Image uploaded to Cloudinary.');
//     } catch (err) {
//       const errorMessage = err instanceof Error ? err.message : 'Failed to create destination';
...
//     // Find country ISO2 from name
//     const country = filteredCountries.find(c => c.name === dest.country);
...
//     setImageUrl(existingImage); // Set URL in case it's a direct URL
...
//       // Check if using URL method (and not uploading a new file)
...
//         console.log('Updated destination with URL:', updatedDestination);

//         await fetchTrendingDestinations();
//         setEditingDestination(null);
//         resetForm();
//         toast.success('Destination updated successfully with image URL!');
//         return;
//       }
...
//       setEditingDestination(null);
//       resetForm();
//       toast.success(imageFile ? 'Destination updated with new image uploaded to Cloudinary!' : 'Destination updated successfully!');
//     } catch (err) {
//       const errorMessage = err instanceof Error ? err.message : 'Failed to update destination';
...
//   const handleDeleteDestination = async (id: string) => {
//     if (!confirm('Are you sure you want to delete this destination? The image will also be deleted from Cloudinary.')) return;
...
//       // Send directly to backend, bypassing Next.js API route
//       const BACKEND_URL = process.env.NEXT_PUBLIC_BACKEND_URL || 'http://localhost:5001';
...
//       await fetchTrendingDestinations();
//       toast.success('Destination and associated image deleted successfully!');
//     } catch (err) {
...
//             Trending Destinations
//           </h1>
//           <p className="text-gray-600">Manage your trending travel destinations with Cloudinary image storage.</p>
//         </div>
//         <button
//           onClick={() => setShowAddForm(true)}
//           className="px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 transition-colors flex items-center gap-2"
...
//                 onChange={(e) => setFormData({ ...formData, slug: e.target.value })}
//                 className="w-full text-gray-900 px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500"
//                 placeholder="e.g., sikkim-highlands-kalimpong"
//                 required
//               />
//             </div>
//             <div>
...
//                 Images will be organized in Cloudinary: paradise-yatra/packages/{formData.category ? formData.category.toLowerCase().replace(/\s+/g, '-') : 'default'}
//               </p>
//             </div>
//             <div>
//               <label className="block text-sm font-medium text-gray-700 mb-2">Duration</label>
...
//             {/* Enhanced Image Upload Section with URL Option */}
//             <div className="md:col-span-2">
//               <label className="block text-sm font-medium text-gray-700 mb-2">
//                 Destination Image * 
//                 <span className="text-xs text-gray-500 ml-2">(Upload to Cloudinary or provide URL)</span>
//               </label>
...
//                           <p className="text-xs text-purple-600 mt-1">Will be uploaded to Cloudinary</p>
//                         </div>
//                         <input
//                           type="file"
//                           className="hidden"
...
//                       💡 Paste a direct image URL (e.g., from Cloudinary, Imgur, or your CDN)
//                     </p>
//                   </div>
//                 )}
//               </div>
...
//             >
//               {submitting && <Loader2 className="w-4 h-4 animate-spin" />}
//               {submitting ? (editingDestination ? "Updating..." : "Creating...") : (editingDestination ? "Update Destination" : "Add Destination")}
//             </button>
//             <button
//               onClick={resetForm}
//               disabled={submitting}
...
//                 <p className="text-xs text-gray-500">
//                   Image stored in Cloudinary: paradise-yatra/packages/{selectedDestination.category.toLowerCase().replace(/\s+/g, '-')}
//                 </p>
//               </div>
//             </div>
//           </div>
        ...
        <div className="bg-gradient-to-r from-blue-500 to-blue-600 text-white p-6 rounded-lg">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold">{trendingDestinations.filter(d => d.isFeatured).length}</p>
            </div>
            <Star className="w-8 h-8 text-blue-200" />
          </div>
        </div>
